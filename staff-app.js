...
function renderQueue() {
  ...
  $('#queue-rows').innerHTML = registrations.length ? registrations.map((item, index) => `<div class="queue-row"><span>${item.queue}</span><span>${item.name}</span><span><small>${item.type}</small></span><span>${item.service}</span><span>${item.time}</span><span>${item.status === 'Waiting' ? `<button class="done-button" data-index="${index}">Mark done</button>` : '<span class="status-pill">DONE</span>'} <button class="delete-button" data-index="${index}">Delete</button></span></div>`).join('') : '<div class="queue-row"><span>No registrations yet.</span></div>';
}
function renderStaffData() {
  ...
  const fillRows = (selector, records, columns) => { $(selector).innerHTML = records.length ? records.map(item => `<div class="queue-row">${columns(item)}<span>${item.status === 'Waiting' ? `<button class="done-button" data-index="${registrations.indexOf(item)}">Mark done</button>` : '<span class="status-pill">DONE</span>'} <button class="delete-button" data-index="${registrations.indexOf(item)}">Delete</button></span></div>`).join('') : '<div class="queue-row"><span>No registrations yet.</span></div>'; };
  ...
  $('#completed-rows').innerHTML = completed.length ? completed.map(item => `<div class="queue-row"><span>${item.queue}</span><span>${item.name}</span><span>${item.type}</span><span>${item.service}</span><span>${item.time}</span><span><span class="status-pill">DONE</span> <button class="delete-button" data-index="${registrations.indexOf(item)}">Delete</button></span></div>`).join('') : '<div class="queue-row"><span>No completed sessions yet.</span></div>';
...
async function deleteRegistration(event) {
  ...
  const item = registrations[Number(event.target.dataset.index)];
  if (!item || !window.confirm(`Delete queue ${item.queue}? The customer can register again.`)) return;
  try { await api(`/api/registrations/${item.id}`, { method: 'DELETE' }); await renderAll(); toast('Registration deleted. The customer can register again.'); } catch { toast('Could not delete the registration.'); }
}
document.querySelectorAll('#queue-rows, #student-rows, #customer-rows, #completed-rows').forEach(table => table.addEventListener('click', deleteRegistration));
